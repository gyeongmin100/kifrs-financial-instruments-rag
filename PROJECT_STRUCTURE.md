...
질문
 ...
 │    └ 모델은 evidence_id만 반환, 출처·원문은 목록에서 조립
 │
 ▼ 결론 / 판단 과정 / 근거
```
...
## 3. 출처 보장 — 검사하지 않고, 애초에 묻지 않는다

출처 변조를 막는 방법은 둘이다. 모델에게 물어본 뒤 대조하거나, **묻지 않거나.** 후자를 택했다.

### 모델에게 받는 것

```python
"evidence": {"type": "array", "items": {"type": "string"}},   # ["E1", "E3"]
```

`citation`과 `statement`는 스키마에서 뺐다. 두 값 모두 검색 단계에서 `prepare_evidence_catalog()`가 만든 목록에 이미 있으므로, 모델에게 되물을 이유가 없었다. **묻지 않은 값은 틀릴 수도, 지어낼 수도 없다.**
...
for text in (conclusion, *reasoning):
    cited.extend(_MARKER_RE.findall(text))       # 본문 마커도 함께 모은다

for evidence_id in dict.fromkeys(cited):          # 중복은 여기서 자연 제거
    source = offered_by_id.get(evidence_id)
    if source is None:
        continue                                  # 모르는 번호는 그 항목만 버린다
    item = {"evidence_id": evidence_id,
            "citation": str(source["citation"]),   # ← 우리 값
            "statement": str(source["statement"])} # ← 우리 값
```

| 모델의 실수 | 이전 | 지금 |
|---|---|---|
| 없는 `E9` 인용 | 답변 전체 폐기 | 그 항목만 제외 |
| 같은 번호 두 번 | 답변 전체 폐기 | 중복 제거 |
| 출처명 변조 | 답변 전체 폐기 | **불가능** (모델 값을 안 씀) |
| 문장에 마커 없음 | 답변 전체 폐기 | 그대로 통과 |
| 본문은 `[E1]` 인용, 목록에선 누락 | 답변 전체 폐기 | 마커에서 찾아 카드 생성 |

목록과 본문 마커를 **합집합**으로 모으는 이유: 둘 중 하나만 보면 한쪽에만 등장한 근거가 카드에서 사라진다. 이 합집합 덕분에 "evidence가 비었다"는 신호가 신뢰할 만해지고, 아래 거절 판정이 그 위에 선다.

`_fallback()`은 남아 있지만 이제 **API 오류와 JSON 파싱 실패에만** 쓰인다. 검증 실패 경로는 없다.

### 왜 검사를 없앴나 — 실측

같은 질문 14개로 측정했다. 거절 4건 중 **3건이 진짜 거절이 아니라 검사에 걸린 것**이었다.

| 질문 | 모델이 실제로 한 일 | 걸린 검사 |
|---|---|---|
| 법인세비용 계산 | 근거 3개 인용, "제1012호 소관"이라 정확히 안내 | 문장 하나에 마커 없음 |
| 오늘 서울 날씨 | "제공된 근거는 K-IFRS 금융상품 내용"이라 구체적 거절 | `답변할 수 없습니다`가 접두사 목록에 없음 |
| 금융부채·지분상품 구분 | 5,149자 작성 중 잘림 | (검사 아님 — `max_output_tokens` 초과) |

셋 다 사용자에게는 똑같은 `근거 부족: 검증된 답변을 생성하지 못했습니다.`로 표시됐다. 모델이 만든 유용한 설명이 버려진 것이다.

| | 전 | 후 |
|---|---|---|
| 답변 | 10/14 | **11/14** |
| 검사로 폐기 | 3건 | **0건** |
| 근거 카드 수 | 질문당 평균 2.2개 | **2.9개** |

### 상태 판정

```python
declined = (
    not (answer.get("evidence") or [])            # 근거를 하나도 안 썼거나
    or conclusion.startswith(_INSUFFICIENT_PREFIXES)  # 스스로 거절했거나
)
```

**접두사만 믿으면 안 된다.** 실측에서 모델은 `답변할 수 없습니다.`라고 썼는데 코드가 찾는 접두사는 `근거 부족:` / `답변 불가:`였다. 검사가 있던 시절에는 검사가 대신 잡아줬지만, 검사를 없애면 이런 거절이 `answered`로 잘못 분류된다. 그래서 "근거를 하나도 인용하지 않았다"를 1순위 신호로 둔다. 프롬프트 규칙 6이 이 동작을 지시하므로 문구보다 구조가 안정적이다.

| status | reason | 발생 조건 |
|---|---|---|
| `answered` | `sufficient` | 근거를 인용했고 거절 접두사도 없음 |
| `insufficient` | `self_declined` | 근거가 비었거나 거절 접두사로 시작 |
...
- 답변 문장이 근거 원문의 **내용**과 일치하는지 판단하는 장치가 없다.
- 답변이 근거의 범위를 넘어 일반화하는지 감지하지 못한다. 이전의 의미 판정이 그 역할을 일부 했다.

이 트레이드오프는 의도적이다. 기각률 60%는 서비스가 성립하지 않는 수준이었다.

### 검토했으나 도입하지 않은 것 — 근거 부족 시 2차 재검색

**다시 꺼내기 전에 이 절을 먼저 읽을 것.** 아래는 측정 결과이지 취향이 아니다.

검토한 안: 1차에서 모델이 `근거 부족:`이라 하면 `REFERS_TO`로 그래프를 확장해 재검색하고 한 번 더 생성한다.

**측정 1 — 확장이 실제로 무엇을 가져오나** (검색가능 Chunk 4,144개 전수)

```
1-hop 참조로 딸려오는 청크 수
  0개      2,687  (65%)   ← 3분의 2는 빈손
  1-3개    1,042  (25%)
  4-8개      230   (6%)
  9개+       185   (4%)
  평균 1.7 · 중앙값 0 · 최대 232
...
`REFERS_TO` 12,796개 중 31%(Standard·ExternalStandard 대상)는 본문 청크로 이어지지 않는다. "제1109호를 참조"처럼 문서 전체를 지칭하기 때문이다.

**측정 2 — 2-hop은 그래프상 자연스럽게 되지 않는다**

```
Paragraph 에서 나가는 REFERS_TO : 0개
REFERS_TO 출발 라벨             : Block 11,017 · Table 1,491 · Footnote 288

Chunk ─DERIVED_FROM─▶ Block ─REFERS_TO─▶ Paragraph ─╳ 여기서 막힘
```

한 번 더 가려면 `Paragraph-[:HAS_BLOCK]->Block-[:REFERS_TO]->Paragraph`로 **구조 관계를 경유**해야 한다. 의미상 "참조의 참조"가 아니라 5-edge 우회이며, 별도 쿼리를 새로 짜야 한다.

**측정 3 — 재검색이 구제할 수 있는 질문이 몇 개인가** (질문 14개)

거절 4건의 실제 원인:

| 원인 | 건수 | 재검색이 고치나 |
|---|---|---|
| 검사 실패 (마커 없음·접두사 불일치) | 2 | ✗ |
| `max_output_tokens` 초과 | 1 | ✗ **오히려 악화** |
| 진짜 근거 부족 | 1 | ○ |

**기각의 원인이 검색 부족이 아니었다.** 검사 제거와 토큰 상향만으로 거절이 4 → 3으로 줄었고, 남은 3건 중 2건은 범위 밖 질문이다.

**결론: 도입하지 않는다.** 이유는 셋이다.

1. 구제 대상이 14건 중 1건이다.
2. 1-hop 확장은 평균 +19.8개, 최대 +63개를 붙인다. 후보가 33개가 되면 `max_candidates`(20)를 넘고, 이미 출력 토큰이 모자라 터진 적이 있는 상태에서 컨텍스트를 3배로 키우는 셈이다. **첫 번째 실패 유형을 더 자주 터뜨린다.**
3. 상한을 걸면 "무엇을 남길지" 정해야 하고, 그것은 §4에서 제거한 재정렬이 하던 일이다. 확장 → 과다 → 걸러냄 → 확장 무의미, 같은 고리가 반복된다.

재도입을 검토한다면 **먼저 거절 원인을 분류해 "진짜 근거 부족"의 비율부터 측정할 것.** 측정 스크립트의 판별 기준은 `evidence`가 비었는지 여부다(범위 밖) 대 근거는 있는데 거절인지(부분 부족).
...
| 인용 검증 | 검증 대신 **모델에게 출처를 묻지 않음** | 검사는 통과/폐기 판단이 필요하지만, 묻지 않으면 판단 자체가 불필요하다 |
...
answer:
  ...
  max_output_tokens: 8000   # 추론 모델은 생각한 토큰도 포함. 3000에서 JSON이 잘렸다
```
...
| 한계 | 내용 |
|---|---|
| **출처는 보장, 해석은 미보장** | 카드의 출처·원문은 DB 값이라 정확하다. 그러나 본문이 그 원문을 올바르게 해석했는지 확인할 장치는 없다 |
| **인용 선택을 검증하지 않는다** | `[E1]`을 붙였다고 그 문단이 실제로 그 문장을 뒷받침한다는 보장은 없다. 사용자가 카드 원문과 대조할 수 있게 한 것이 대응책이다 |
| **범위 초과를 감지하지 못한다** | 답변이 근거 밖으로 일반화해도 막을 장치가 없다 |
| **거절 사유가 화면에 안 보인다** | 모델이 "제1012호 소관"처럼 구체적으로 안내해도 프론트엔드가 고정 문구로 덮는다 |
| **결과에 변동성이 있다** | 답변 생성 LLM에 `temperature`를 설정하지 않았다. 재정렬 제거로 변동 폭은 크게 줄었지만 0은 아니다 |
| **그래프 대부분이 유휴** | `REFERS_TO` 12,796개, `MENTIONS` 17,712개는 적재만 되어 있다 (§4 참조) |
...
6. **게이트를 다시 추가하려면 기각률을 먼저 측정한다.** §4의 질문 세트가 기준선이다.
7. **모델에게 우리가 이미 아는 값을 되묻지 않는다.** `citation`·`statement`를 다시 받으면 대조가 필요해지고, 대조는 폐기 판단을 낳고, 폐기 판단은 멀쩡한 답변을 죽인다. §3 참조.
8. **거절이 늘면 원인부터 분류한다.** `evidence`가 비었으면 범위 밖, 근거가 있는데 거절이면 부분 부족이다. 검색을 손대기 전에 후자의 비율을 잰다. 실측에서 거절 4건 중 검색 문제는 1건뿐이었다.
9. **`max_output_tokens`를 비용 절감용으로 낮추지 않는다.** 한도일 뿐 사용량이 아니다. 부족하면 JSON이 문장 중간에서 끊겨 답변 전체를 잃는다. 추론 모델은 생각한 토큰도 이 한도에 포함한다.
